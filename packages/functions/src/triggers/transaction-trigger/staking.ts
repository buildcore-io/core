import { PgTokenStatsUpdate, PgTransaction, build5Db } from '@build-5/database';
...
export const onStakingConfirmed = async (billPayment: PgTransaction) => {
  const stakeDocRef = build5Db().doc(COL.STAKE, billPayment.payload_stake!);
  ...
  const updateData: PgTokenStatsUpdate = {
    ...
    [`stakes_${stake.type}_totalAmount`]: build5Db().inc(stake.amount),
    [`stakes_${stake.type}_value`]: build5Db().inc(stake.value),
    [`stakes_${stake.type}_totalValue`]: build5Db().inc(stake.value),
