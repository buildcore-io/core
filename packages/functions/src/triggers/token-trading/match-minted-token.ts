...
export const matchMintedToken = async (
  ...
  const buyOrderTran = (await build5Db().doc(COL.TRANSACTION, buy.orderTransactionId!).get())!;
  const sellOrderTran = (await build5Db().doc(COL.TRANSACTION, sell.orderTransactionId!).get())!;
