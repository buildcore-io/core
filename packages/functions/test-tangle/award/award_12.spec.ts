...
describe('Create award, base', () => {
  ...
  it('Should create, fund and participate in award, return to sender address when funding address is missing', async () => {
    ...
    expect(awardData.fundingAddress).toBe(address.bech32);
    await awardDocRef.update({ fundingAddress: build5Db().deleteField() });
    ...
    const snap = await burnAliasQuery.get<Transaction>();
    expect(snap[0].payload.targetAddress).toBe(address.bech32);
