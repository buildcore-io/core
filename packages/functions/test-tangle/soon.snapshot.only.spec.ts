...
import { soonSnapshot } from '../scripts/dbUpgrades/soon.snapshot';
...
describe('Soon snapshot test', () => {
  ...
  beforeAll(async () => {
    ...
    await database().getCon().raw(`
      CREATE INDEX IF NOT EXISTS milestone_smr_transactions_ord ON milestone_smr_transactions ("createdOn", uid);
      CREATE INDEX IF NOT EXISTS milestone_rms_transactions_ord ON milestone_rms_t2_transactions ("createdOn", uid);
    `);
  });
  ...
  beforeEach(async () => {
    ...
    await database()
      ...
      .update({ [`${network}Address`]: address.bech32 });
    const space = await testEnv.createSpace(guardian);

    await database()
      .getCon()(getMilestoneCol(Network.RMS) + '_' + SUB_COL.TRANSACTIONS)
      .delete();
    await database().getCon()(getMilestoneCol(Network.RMS)).delete();
    await database().getCon()(COL.AIRDROP).delete();
    await database().getCon()(COL.TOKEN).delete();
  ...
  it('Should get 100 token from vault and claim it', async () => {
    ...
    let snapshot = await database().doc(COL.SOON_SNAP, vaultAddress).get();
    ...
    snapshot = await database().doc(COL.SOON_SNAP, address.bech32).get();
    ...
    const docRef = database().doc(COL.SOON_SNAP, address.bech32);
  ...
  it('Should send 50 to random user, keep 50', async () => {
    ...
    const snapshot = await database().doc(COL.SOON_SNAP, targetAddress.bech32).get();
  ...
  it('Airdrop 2 * 10 token, unclaimed', async () => {
    ...
    mockWalletReturnValue(guardian, { token: tokenId, drops });
    ...
    mockWalletReturnValue(guardian, { token: tokenId, drops });
    order = await testEnv.wrap<Transaction>(WEN_FUNC.airdropMintedToken);
  ...
  it('Airdrop 2 * 10 token, claim half, do not count twice', async () => {
    ...
    const claimOrder = (await orderQuery.get())[0];
    await wallet.send(
      tmp,
    ...
    await wait(async () => {
      const snap = await database()
        .collection(COL.AIRDROP)
        .whereIn('member', [targetAddress.bech32, tmp.bech32])
        .get();
    ...
    snapshot = await database().doc(COL.SOON_SNAP, targetAddress.bech32).get();
    expect(snapshot?.count).toBe(20);
    snapshot = await database().doc(COL.SOON_SNAP, tmp.bech32).get();
    expect(snapshot?.count).toBe(20);
  });

  it('Airdrop to member with no validated address', async () => {
    const member = '0x69252ebdc3b77624c6e640a81a086aaa720734d3';
    await database().doc(COL.MEMBER, member).upsert({ rmsAddress: '' });
    ...
    const snapshot = await database().doc(COL.SOON_SNAP, member).get();
  ...
  it('Airdrop to members 0x address', async () => {
    ...
    await database()
      ...
      .upsert({ [`${Network.RMS}Address`]: tmp.bech32 });
    const drops = [{ count: 10, recipient: member, vestingAt: dayjs().add(2, 'M').toDate() }];
...
const blockInDd = async (blockId: string) => {
  await wait(async () => {
    const snap = await database()
      .collection(COL.MILESTONE_RMS, undefined, SUB_COL.TRANSACTIONS)
      .get();
