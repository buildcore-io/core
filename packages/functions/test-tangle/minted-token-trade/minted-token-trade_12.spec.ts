...
describe('Token minting', () => {
  ...
  it('Half fulfill buy and make it expired it', async () => {
    ...
    const buyer = <Member>(
      (await admin.firestore().doc(`${COL.MEMBER}/${helper.buyer!}`).get()).data()
    );
  ...
  it('Half fulfill sell and make it expired it', async () => {
    ...
    const credit = <Transaction>(
      (await admin.firestore().doc(`${COL.TRANSACTION}/${sell.creditTransactionId}`).get()).data()
    );
    expect(credit.member).toBe(helper.seller);
    const seller = <Member>(
      (await admin.firestore().doc(`${COL.MEMBER}/${helper.seller!}`).get()).data()
    );
