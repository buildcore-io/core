...
export class MilestoneListener {
  ...
  private run = async () => {
    ...
    for (let i = 0; i < 864000; ++i) {
      ...
      const query = onlineDb
        .collection(getCollectionRef(this.network))
  ...
  private onMilestoneChange = async (
    network: Network,
    milestone: any,
    subColl: SUB_COL,
    wallet: SmrWallet,
  ) => {
    const transSnap = await onlineDb
      .collection(`${getCollectionRef(this.network)}/${milestone.milestone}/${subColl}`)
      .get();
    const docs = [] as any[];
    ...
    const mDocRef = admin
      .firestore()
      .doc(`${getCollectionRef(this.network)}/${milestone.milestone}`);
    if (!(await mDocRef.get()).exists) {
      await mDocRef.set(cleanTimestamps(milestone));
    }
    for (const doc of docs) {
      const tranDocRef = admin
        .firestore()
        .doc(`${getCollectionRef(this.network)}/${milestone.milestone}/${subColl}/${doc.id}`);
...
const cleanTimestamps = (data: any) =>
