...
describe('Test custom constraints', () => {
  ...
  it.each([PublicCollections.AVATARS, PublicCollections.BADGES])(
    ...
    async (collection: PublicCollections) => {
      ...
      uids.forEach((uid) => batch.create(build5Db().doc(`${collection}/${uid}`), { name: uid }));
      await batch.commit();
  ...
  it('Get all should only return none hidden nfts', async () => {
    ...
    const batch = build5Db().batch();
    uids.forEach((uid, i) =>
      batch.create(build5Db().doc(`${PublicCollections.NFT}/${uid}`), {
        name: uid,
        space,
        hidden: i > 1,
      }),
  ...
  it('Should not return hidden nft', async () => {
    ...
    build5Db().doc(`${PublicCollections.NFT}/${uid}`).create({ name: uid, space, hidden: true });
  ...
  it('Get all should not return order transactions', async () => {
    ...
    const batch = build5Db().batch();
    uids.forEach((uid, i) =>
      batch.create(build5Db().doc(`${PublicCollections.TRANSACTION}/${uid}`), {
        name: uid,
        space,
  ...
  it.each([COL.MNEMONIC, COL.SYSTEM])(
    ...
    async (collection: COL) => {
      const uid = getRandomEthAddress();
      await build5Db().doc(`${collection}/${uid}`).create({ name: 'asd' });
