...
describe('Nft bidding with extended auction', () => {
  const h = new Helper();
  let now: dayjs.Dayjs;
  let nftDocRef: IDocument<Nft, PgNft, PgNftUpdate>;
  ...
  it('Should throw, extended auction lenght must be greater then auction lenght', async () => {
    const auctionData = { ...dummyAuctionData(h.nft.uid), extendedAuctionLength: 60000 * 3 };
    mockWalletReturnValue(h.member, auctionData);
    await expectThrow(testEnv.wrap(WEN_FUNC.setForSaleNft), WenError.invalid_params.key);
  });
