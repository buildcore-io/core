...
async function main() {
    ...
    try {
        response = await https(origin).project(Build5ApiKey[origin]).dataset(Dataset.STAMP).stamp({
    ...
    console.log('Sent: ', response.payload.amount, ' to ', response.payload.targetAddress, ', full order object: ', response);
}
