...
export class NftDepositComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.transaction$.pipe(untilDestroyed(this)).subscribe((val) => {
      ...
      if (val && val.type === TransactionType.PAYMENT && val.payload.reconciled === true) {
        this.pushToHistory(val, val.uid + '_payment_received', val.createdOn, $localize`NFT received.`, (<any>val).payload?.chainReference);
      }
      ...
      if (val && val.type === TransactionType.CREDIT && val.payload.reconciled === true && !val.payload?.walletReference?.chainReference) {
        this.pushToHistory(val, val.uid + '_false', val.createdOn, $localize`Invalid NFT received.`);
      }

      if (val && val.type === TransactionType.CREDIT && val.payload.reconciled === true && val.payload?.walletReference?.chainReference) {
        this.pushToHistory(val, val.uid + '_true', dayjs(), $localize`Invalid NFT refunded.`, val.payload?.walletReference?.chainReference);
  ...
  public async proceedWithDeposit(): Promise<void> {
    ...
    await this.auth.sign(params, (sc, finish) => {
      this.notification.processRequest(this.nftApi.depositNft(sc), 'Order created.', finish).subscribe((val: any) => {
