...
export class CollectionHighlightCardComponent implements OnDestroy {
  ...
  private fetchData(): void {
    ...
    this.collections.forEach((collection) => {
      ...
      this.subscriptions$.push(this.spaceApi.listen(collection?.space).pipe(untilDestroyed(this)).subscribe(space$));
      this.subscriptions$.push(this.nftApi.lowToHighCollection(collection?.uid, undefined, 1)?.pipe(map((obj: Nft[]) => obj[0]), untilDestroyed(this)).subscribe(cheapestNft$));
    });
  }
