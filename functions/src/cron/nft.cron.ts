...
const finalizeNftAuction = (nftId: string) => admin.firestore().runTransaction(async (transaction) => {
...
export const finalizeAllNftAuctions = async () => {
  const snap = await admin.firestore().collection(COL.NFT).where('auctionTo', '<=', dayjs().toDate()).get();
