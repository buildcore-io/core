...
export const assertHasAccess = async (spaceId: string, member: string | undefined, access: Access, accessAwards: string[], accessCollections: string[]) => {
  ...
  if (access === Access.MEMBERS_ONLY) {
    if (!(await admin.firestore().doc(`${COL.SPACE}/${spaceId}/${SUB_COL.MEMBERS}/${member}`).get()).exists) {
      throw throwInvalidArgument(WenError.you_are_not_part_of_space);
    }
  }

  if (access === Access.GUARDIANS_ONLY) {
    if (!(await admin.firestore().doc(`${COL.SPACE}/${spaceId}/${SUB_COL.GUARDIANS}/${member}`).get()).exists) {
  ...
  if (access === Access.MEMBERS_WITH_BADGE) {
    ...
    if (snapshot.size && accessAwards.length) {
      for (const doc of snapshot.docs) {
        const data = doc.data()
        if (accessAwards.includes(data.payload.award) && !includedBadges.includes(data.payload.award)) {
  ...
  if (access === Access.MEMBERS_WITH_NFT_FROM_COLLECTION) {
    ...
    if (snapshot.size > 0 && accessCollections?.length) {
      for (const doc of snapshot.docs) {
        const data = doc.data()
        if (accessCollections.includes(data.collection) && !includedCollections.includes(data.collection)) {
