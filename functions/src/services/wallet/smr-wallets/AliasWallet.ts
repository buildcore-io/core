import { Bech32Helper, GOVERNOR_ADDRESS_UNLOCK_CONDITION_TYPE, IAliasOutput, IndexerPluginClient, STATE_CONTROLLER_ADDRESS_UNLOCK_CONDITION_TYPE, TransactionHelper } from "@iota/iota.js-next";
...
export class AliasWallet {
  ...
  public mintAlias = async (transaction: Transaction, params: SmrParams) => {
    ...
    const essence = packEssence(inputs, inputsCommitment, remainderAmount ? [aliasOutput, remainder] : [aliasOutput], this.wallet, params)
  ...
  public changeAliasOwner = async (transaction: Transaction, params: SmrParams) => {
    ...
    const aliasOutputs = await this.getAliasOutputs(sourceAddress.bech32, sourceMnemonic.consumedAliasOutputIds)
  ...
  public getAliasOutputs = async (governorBech32: string, prevConsumedAliasOutputId: string[] = []) => {
    const indexer = new IndexerPluginClient(this.wallet.client)
    const outputIds = isEmpty(prevConsumedAliasOutputId) ? (await indexer.aliases({ governorBech32 })).items : prevConsumedAliasOutputId
