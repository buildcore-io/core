...
export const matchTradeOrder = async (tradeOrder: TokenTradeOrder) => {
  ...
  if (tradeOrder.type === TokenTradeOrderType.BUY) {
    do {
      lastDoc = await runTradeOrderMatching(query, matcher, postMatchActions, lastDoc, token, tradeOrder.uid, false)
    } while (lastDoc)
  }
}
...
const runTradeOrderMatching = async (
  ...
  admin.firestore().runTransaction(async (transaction) => {
    ...
    for (const trade of trades) {
      ...
      const price = invertedPrice ? (isSell ? prevBuy.price : prevSell.price) : (isSell ? prevSell.price : prevBuy.price)
      const triggeredBy = isSell ? TokenTradeOrderType.SELL : TokenTradeOrderType.BUY
      const { purchase, buyerCreditId } = await matcher(transaction, token, prevBuy, prevSell, price, triggeredBy)
      if (!purchase) {
        continue
      }
...
const getMatcher = (token: Token) => {
  switch (token.status) {
    case TokenStatus.BASE: return matchBaseToken
    case TokenStatus.MINTED: return matchMintedToken
    default: return matchSimpleToken
  }
}

const getPostMatchActions = (token: Token) => {
  switch (token.status) {
    case TokenStatus.BASE: return undefined
    case TokenStatus.MINTED: return undefined
    default: return updateSellLockAndDistribution
  }
}
