...
const onCollectionMinted = async (transaction: Transaction) => {
  ...
  const orders = await createNftMintingOrders(transaction, inputParams)
  const nftsToMint = orders.reduce((acc, act) => acc + act.payload.nfts.length, 0)
  await saveCollectionMintingData(transaction, milestoneTransaction.blockId, collectionOutputId, nftsToMint)

  const promises = orders.map(order => admin.firestore().doc(`${COL.TRANSACTION}/${order.uid}`).create(order))
...
const createNftMintingOrders = async (transaction: Transaction, inputPramas: MintNftInputParams) => {
  ...
  do {
    ...
    do {
      const nftOutputs = packNfts(nfts.slice(0, actNftsToMint), collection, nftWallet, tmpAddress, collectionNftId)
...
const packNfts = (nfts: Nft[], collection: Collection, nftWallet: NftWallet, address: AddressDetails, collectionNftId: string) =>
  nfts.map((nft) => nftWallet.packNft(nft, collection, address.bech32, address, collectionNftId))
