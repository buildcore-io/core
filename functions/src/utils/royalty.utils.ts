...
export const getRoyaltyFees = async (amount: number, memberPercentage: number | undefined, isTokenPurchase = false) => {
  ...
  if (isTokenPurchase) {
    const royaltiesSpaceOne = Number(bigDecimal.floor(bigDecimal.multiply(amount, percentage / 100)))
...
const getTokenPurchaseFeePercentage = async (memberFeePercentage: number | undefined, isTokenPurchase = false) => {
  if (memberFeePercentage !== undefined) {
    return memberFeePercentage
  }
  const systemConfig = <SystemConfig | undefined>(await admin.firestore().doc(`${COL.SYSTEM}/${SYSTEM_CONFIG_DOC_ID}`).get()).data()
  const systemPercentage = isTokenPurchase ? systemConfig?.tokenPurchaseFeePercentage : systemConfig?.tokenTradingFeePercentage
