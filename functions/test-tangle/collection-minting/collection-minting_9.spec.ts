...
describe('Collection minting', () => {
  ...
  it('Should throw, member has no valid address', async () => {
    await admin.firestore().doc(`${COL.MEMBER}/${helper.guardian}`).update({ validatedAddress: {} })
  ...
  it('Should throw, space has no valid address', async () => {
    await admin.firestore().doc(`${COL.SPACE}/${helper.space!.uid}`).update({ validatedAddress: {} })
    mockWalletReturnValue(helper.walletSpy, helper.guardian!, {
      collection: helper.collection, network: helper.network, unsoldMintingOptions: UnsoldMintingOptions.KEEP_PRICE
    })
  ...
  it('Should throw, royalty space has no valid address', async () => {
    await admin.firestore().doc(`${COL.SPACE}/${helper.royaltySpace!.uid}`).update({ validatedAddress: {} })
    mockWalletReturnValue(helper.walletSpy, helper.guardian!, {
      collection: helper.collection, network: helper.network, unsoldMintingOptions: UnsoldMintingOptions.KEEP_PRICE
    })
    await expectThrow(testEnv.wrap(mintCollectionOrder)({}), WenError.space_must_have_validated_address.key)
