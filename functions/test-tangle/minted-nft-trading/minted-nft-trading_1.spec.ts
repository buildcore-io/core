...
describe('Minted nft trading', () => {
  ...
  it('Should bid twice on minted nft and withdraw it', async () => {
    ...
    requestFundsFromFaucet(Network.RMS, bidOrder.payload.targetAddress, MIN_IOTA_AMOUNT)
    ...
    requestFundsFromFaucet(Network.RMS, bidOrder2.payload.targetAddress, 2 * MIN_IOTA_AMOUNT)

    await wait(async () => {
      const nft = <Nft>(await admin.firestore().doc(`${COL.NFT}/${helper.nft!.uid}`).get()).data()
      const payment = (await admin.firestore().collection(COL.TRANSACTION)
    ...
    await admin.firestore().doc(`${COL.NFT}/${helper.nft!.uid}`).update({ auctionTo: dateToTimestamp(dayjs().subtract(1, 'm').toDate()) })
