...
const outputFile = '../.github/workflows/functions-unit-tests_tangle.yml'

function getJobForFile(filePath) {
  const fileName = filePath.replaceAll('.', '-').replaceAll('/', '-')
  fs.appendFileSync(outputFile, `  functions_unit_tests_tangle_emulator_${fileName}:\n`);
  ...
  fs.appendFileSync(outputFile, `      - run: ./node_modules/.bin/firebase emulators:exec --only functions,firestore "npm run test-tangle -- --findRelatedTests ${filePath}" --project dev\n\n`);
}

try {
  fs.writeFileSync(outputFile, 'name: Functions Emulated Unit Tests - Tangle\n');
