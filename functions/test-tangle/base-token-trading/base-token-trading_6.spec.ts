...
import { TokenPurchase, TokenTradeOrderType } from "../../interfaces/models";
...
describe('Base token trading', () => {
  ...
  it('Should fulfill sell with highest buy', async () => {
    ...
    await requestFundsFromFaucet(helper.targetNetwork, buyOrder.payload.targetAddress, buyOrder.payload.amount)
    ...
    await requestFundsFromFaucet(helper.targetNetwork, buyOrder2.payload.targetAddress, buyOrder2.payload.amount)

    await wait(async () => {
      const snap = await admin.firestore().collection(COL.TOKEN_MARKET).where('owner', '==', helper.buyer!.uid).get()
      return snap.size === 2
    })
    ...
    await requestFundsFromFaucet(helper.sourceNetwork, sellOrder.payload.targetAddress, sellOrder.payload.amount)
