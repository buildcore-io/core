...
describe('Base token trading', () => {
  ...
  it('Should not fill buy, dust and order not fulfilled', async () => {
    ...
    mockWalletReturnValue(helper.walletSpy, helper.buyer!.uid, { token: helper.token, count: MIN_IOTA_AMOUNT + 1, price: 2, type: TokenTradeOrderType.BUY })
    const buyOrder = await testEnv.wrap(tradeToken)({})
    await requestFundsFromFaucet(helper.targetNetwork, buyOrder.payload.targetAddress, 2 * (MIN_IOTA_AMOUNT + 1))
    ...
    const purchase = await admin.firestore().collection(COL.TOKEN_PURCHASE).where('token', '==', helper.token).get()
