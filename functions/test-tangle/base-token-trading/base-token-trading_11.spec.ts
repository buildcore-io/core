...
import { TokenPurchase, TokenTradeOrderType, Transaction, TransactionType } from "../../interfaces/models";
...
describe('Base token trading', () => {
  ...
  it.each([false, true])('Should not create royalty payments, zero percentage fee', async (isMember: boolean) => {
    if (isMember) {
      await admin.firestore().doc(`${COL.MEMBER}/${helper.seller!.uid}`).update({ tokenTradingFeePercentage: 0 })
    } else {
    ...
    const tradesQuery = admin.firestore().collection(COL.TOKEN_MARKET).where('token', '==', helper.token)
    await wait(async () => {
    ...
    const purchaseQuery = admin.firestore().collection(COL.TOKEN_PURCHASE).where('token', '==', helper.token)
    await wait(async () => {
    ...
    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT * 2 && bp.payload.owner === helper.seller!.uid)
    expect(billPaymentToSeller).toBeDefined()

    const billPaymentToBuyer = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT && bp.payload.owner === helper.buyer!.uid)
  ...
  it('Should create royalty payments only with dust', async () => {
    ...
    mockWalletReturnValue(helper.walletSpy, helper.buyer!.uid, { token: helper.token, count: MIN_IOTA_AMOUNT, price: 2.001, type: TokenTradeOrderType.BUY })
    const buyOrder = await testEnv.wrap(tradeToken)({})
    await requestFundsFromFaucet(helper.targetNetwork, buyOrder.payload.targetAddress, 2.001 * MIN_IOTA_AMOUNT)
    ...
    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT * 2 - billPaymentToSpaceOne?.payload?.storageReturn?.amount!)
    expect(billPaymentToSeller).toBeDefined()
  ...
  it('Should create royalty payments for different percentage', async () => {
    await admin.firestore().doc(`${COL.MEMBER}/${helper.seller!.uid}`).update({ tokenTradingFeePercentage: 1 })
    mockWalletReturnValue(helper.walletSpy, helper.seller!.uid, { token: helper.token, count: MIN_IOTA_AMOUNT, price: 2, type: TokenTradeOrderType.SELL })
    const sellOrder = await testEnv.wrap(tradeToken)({})
    await requestFundsFromFaucet(helper.sourceNetwork, sellOrder.payload.targetAddress, MIN_IOTA_AMOUNT)
    ...
    const billPaymentToSpaceOne = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT * 2 * 0.01 * 0.1 + 46800)
    expect(billPaymentToSpaceOne).toBeDefined()

    const billPaymentToSpaceTwo = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT * 2 * 0.01 * 0.9 + 46800)
    expect(billPaymentToSpaceTwo).toBeDefined()

    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT * 2 * 0.99 -
