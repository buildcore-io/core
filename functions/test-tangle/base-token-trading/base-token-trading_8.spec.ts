...
describe('Base token trading', () => {
  ...
  it('Should fulfill many sells with buy', async () => {
    ...
    await requestFundsForManyFromFaucet(Network.ATOI, orders.map(o => ({ toAddress: o.payload.targetAddress, amount: o.payload.amount })))
    ...
    mockWalletReturnValue(helper.walletSpy, helper.buyer!.uid, { token: helper.token, count: count * MIN_IOTA_AMOUNT, price: 1, type: TokenTradeOrderType.BUY })
