...
describe('Base token trading', () => {
  ...
  it.each([TokenTradeOrderType.SELL, TokenTradeOrderType.BUY])('Should credit expired trade', async (type: TokenTradeOrderType) => {
    ...
    mockWalletReturnValue(helper.walletSpy, member.uid, { token: helper.token, count: MIN_IOTA_AMOUNT, price: 1, type })
    const tradeOrder = await testEnv.wrap(tradeToken)({})
    await requestFundsFromFaucet(network, tradeOrder.payload.targetAddress, tradeOrder.payload.amount)

    const tradeQuery = admin.firestore().collection(COL.TOKEN_MARKET).where('token', '==', helper.token)
    await wait(async () => {
      const snap = await tradeQuery.get()
      return snap.size > 0
    })
    let trade = <TokenTradeOrder>(await tradeQuery.get()).docs[0].data()
    await admin.firestore().doc(`${COL.TOKEN_MARKET}/${trade.uid}`).update({ expiresAt: dateToTimestamp(dayjs().subtract(1, 'd')) })
