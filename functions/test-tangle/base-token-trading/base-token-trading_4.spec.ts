...
describe('Base token trading', () => {
  ...
  it('Should fulfill buy with two sells', async () => {
    ...
    await requestFundsFromFaucet(helper.sourceNetwork, sellOrder2.payload.targetAddress, MIN_IOTA_AMOUNT)

    mockWalletReturnValue(helper.walletSpy, helper.buyer!.uid, { token: helper.token, count: 2 * MIN_IOTA_AMOUNT, price: 1, type: TokenTradeOrderType.BUY })
