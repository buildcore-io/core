...
describe('Base token trading', () => {
  ...
  it('Should fulfill buy with lowest sell', async () => {
    ...
    await requestFundsFromFaucet(helper.sourceNetwork, sellOrder2.payload.targetAddress, sellOrder2.payload.amount)

    await wait(async () => {
      const snap = await admin.firestore().collection(COL.TOKEN_MARKET).where('owner', '==', helper.seller!.uid).get()
      return snap.size === 2
    })
