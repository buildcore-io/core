...
import { Member, Network, Space, Token, TokenStatus, TokenTradeOrderType, Transaction } from "../../interfaces/models";
...
import { createMember, createRoyaltySpaces, createSpace, getRandomSymbol, mockWalletReturnValue, wait } from "../../test/controls/common";
...
export class Helper {
  ...
  public beforeEach = async () => {
    ...
    const sellerDoc = <Member>(await admin.firestore().doc(`${COL.MEMBER}/${this.seller}`).get()).data()
    this.sellerAddress = await this.walletService!.getAddressDetails(getAddress(sellerDoc, this.network!))
    await requestFundsFromFaucet(this.network, this.sellerAddress.bech32, 20 * MIN_IOTA_AMOUNT)
    await requestMintedTokenFromFaucet(this.walletService!, this.sellerAddress, this.token.mintingData?.tokenId!, VAULT_MNEMONIC)

    this.buyer = await createMember(this.walletSpy)
    const buyerDoc = <Member>(await admin.firestore().doc(`${COL.MEMBER}/${this.buyer}`).get()).data()
  ...
  public createSellTradeOrder = async (count = 10, price = MIN_IOTA_AMOUNT) => {
    ...
    await this.walletService!.send(this.sellerAddress!, sellOrder.payload.targetAddress, 0, {
      nativeTokens: [{ amount: HexHelper.fromBigInt256(bigInt(count)), id: this.token!.mintingData?.tokenId! }]
    })
    await wait(async () => {
      const snap = await admin.firestore().collection(COL.TOKEN_MARKET).where('orderTransactionId', '==', sellOrder.uid).get()
      return snap.size === 1
    })
    await MnemonicService.store(this.sellerAddress!.bech32, this.sellerAddress!.mnemonic, this.network!)
    return sellOrder
  }

  public createBuyOrder = async (count = 10, price = MIN_IOTA_AMOUNT) => {
    mockWalletReturnValue(this.walletSpy, this.buyer!, { token: this.token!.uid, count, price, type: TokenTradeOrderType.BUY })
    const buyOrder: Transaction = await testEnv.wrap(tradeToken)({})
    await requestFundsFromFaucet(this.network, buyOrder.payload.targetAddress, buyOrder.payload.amount)
    await wait(async () => {
...
export const saveToken = async (space: string, guardian: string, walletService: SmrWallet, tokenId = MINTED_TOKEN_ID) => {
...
export const dummyTokenId = '0x080c409a8c0ffa795676e55f24e0223e5b48dbe2b1bc4314140333543b5e7e8d360100000000'

export const VAULT_MNEMONIC = 'protect bracket walk twelve amount search stone call dress decade arrow flower tray enroll smart day oil twist lumber write denial canvas crunch select'
export const MINTED_TOKEN_ID = '0x080070e0465a4f4ccd4a151e74426379b1fdf41f15aa14188ebed33e60aef9dea90100000000'
