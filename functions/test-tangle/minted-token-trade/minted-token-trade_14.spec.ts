...
describe('Token minting', () => {
  ...
  it('Should send dust to space', async () => {
    ...
    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === MIN_IOTA_AMOUNT * 20 * 0.975 -
  ...
  it('Should not fill buy, balance would be less then MIN_IOTA_AMOUNT and order not fulfilled', async () => {
    await requestMintedTokenFromFaucet(helper.walletService!, helper.sellerAddress!, helper.token!.mintingData?.tokenId!, VAULT_MNEMONIC, 180)
