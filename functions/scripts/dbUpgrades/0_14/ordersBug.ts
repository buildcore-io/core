...
export const setStatusOnAllDocs = async (collection: COL) => {
  ...
  do {
    let query = db.collection(collection).where('type', '==', TransactionType.ORDER).limit(BATCH_LIMIT)
    if (lastDoc) {
      query = query.startAfter(lastDoc)
    }
    const snap = await query.get()
    const promises = snap.docs.map((doc) => {
      if (doc.data().linkedTransactions?.length > 10 && doc.data().createdOn.toDate() > (new Date('2022-09-27'))) {
        console.log(
          'Tran ' + doc.data().createdOn.toDate() + ', ' + collection + ' id: ' + doc.data().uid + ', ' + doc.data().type + ', ' +
          doc.data().payload.targetAddress + ', ' +
          doc.data().payload.type + ', ' + doc.data().member + '...' + doc.data().linkedTransactions?.length);
