...
import { DEF_WALLET_PAY_IN_PROGRESS } from '../interfaces/config';
...
db.collection('transaction').where("payload.walletReference.chainReference", "==", null).where("payload.walletReference.error", "==", "Error: You must specify some inputs").get().then(async (ss) => {
  for (const t of ss.docs) {
    for (const m of (t.data().payload.walletReference.chainReferences || [])) {
      if (m && !m.startsWith(DEF_WALLET_PAY_IN_PROGRESS)) {
        await db.collectionGroup(SUB_COL.TRANSACTIONS).where('messageId', '==', m).get().then(async (sss) => {
          if (sss.size >= 1) {
