    ...
    db.collection('nft').where('collection', '==', col.data().uid).get().then(async (snapshot2) => {
      ...
      for (const nft of snapshot2.docs) {
        ...
        if (nft.data().approved !== col.data().approved) {
          // i++;
          console.log(nft.data().approved, col.data().approved, nft.data().uid, ' should be approved, collection name:', col.data().name);
          update.approved = col.data().approved;
        }

        if (nft.data().rejected !== col.data().rejected) {
          // ii++;
          console.log(nft.data().rejected, col.data().rejected, nft.data().uid, ' should be rejected');
