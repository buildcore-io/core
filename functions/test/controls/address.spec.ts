...
import { createMember, createSpace, expectThrow, milestoneProcessed, mockWalletReturnValue, submitMilestoneFunc, validateMemberAddressFunc, validateSpaceAddressFunc, wait } from './common';
...
describe('Address validation test', () => {
    ...
    it('Should throw, not guardian', async () => {
        ...
        await expectThrow(testEnv.wrap(validateAddress)({}), WenError.you_are_not_guardian_of_space.key)
    })
    ...
    it('Should throw,space already has valid address', async () => {
        await admin.firestore().doc(`${COL.SPACE}/${space}`).update({ validatedAddress: {[Network.IOTA]: 'someaddress'} })
        mockWalletReturnValue(walletSpy, member, { space });
        await expectThrow(testEnv.wrap(validateAddress)({}), WenError.space_already_have_validated_address.key)
    })
