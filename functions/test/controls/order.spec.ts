...
import { createMember, createSpace, expectThrow, milestoneProcessed, mockIpCheck, mockWalletReturnValue, submitMilestoneFunc, submitMilestoneOutputsFunc } from './common';
...
describe('Ordering flows', () => {
  ...
  it('One collection, one classic NFT, one purchase & paid for', async () => {
    // Create collection.
    const price = 100;
    const collection: Collection = await createCollectionFunc(member, dummyCollection(space, CollectionType.CLASSIC, 0.5, price));
  ...
  it('One collection, one classic NFT, one purchase & paid for and try again', async () => {
    ...
    await expectThrow(submitOrderFunc(member, { collection: collection.uid, nft: nft.uid }), WenError.nft_not_available_for_sale.key)
  });

  it('One collection, generated NFT, one purchase and pay', async () => {
    const price = 100;
    const collection = await createCollectionFunc(member, dummyCollection(space, CollectionType.GENERATED, 0.5, price));
  ...
  it('one collection, generated NFT, one purchase and pay - fail no longer available', async () => {
    ...
    await expectThrow(submitOrderFunc(member, { collection: collection.uid }), WenError.no_more_nft_available_for_sale.key)
  });
  ...
  it('One collection, generated NFT, ' + batchSize + ' Nfts should equal to ' + batchSize + ' purchases + payment in multiple milestone', async () => {
    ...
    for (const o of orders) {
      const milestone = await submitMilestoneOutputsFunc([{ amount: o.payload.amount, address: o.payload.targetAddress }]);
      await milestoneProcessed(milestone.milestone, milestone.tranId);
    }
  ...
  it('One collection, generated NFT, one purchase for generated NFT directly. It must be sold.', async () => {
    ...
    const collection = await createCollectionFunc(member, dummyCollection(space, CollectionType.GENERATED, 0.5, price));
    const nft = await createNftFunc(member, dummyNft(collection, price));
    await expectThrow(submitOrderFunc(member, { collection: collection.uid, nft: nft.uid }), WenError.generated_spf_nft_must_be_sold_first.key)
  });
  ...
  it('Order should fail, country blocked for token', async () => {
    ...
    const collection = await createCollectionFunc(member, dummyCollection(space, CollectionType.CLASSIC, 0.5, price));
    const nft = await createNftFunc(member, dummyNft(collection, price));
    mockIpCheck(true, { default: ['USA'], [nft.uid]: ['USA', 'HU'] }, { countryCode: 'HU', privacy: { vpn: false } })
    await expectThrow(submitOrderFunc(member, { collection: collection.uid, nft: nft.uid }), WenError.blocked_country.key)
  })

  it('Order should fail, not blocked but vpn', async () => {
    const price = 100;
    const collection = await createCollectionFunc(member, dummyCollection(space, CollectionType.CLASSIC, 0.5, price));
    const nft = await createNftFunc(member, dummyNft(collection, price));
