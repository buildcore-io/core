...
describe('Trade trigger', () => {
  ...
  it('Should fulfill sell with two buy', async () => {
    ...
    await wait(async () => {
      const sellSnap = await admin.firestore().collection(COL.TOKEN_MARKET)
        .where('type', '==', TokenTradeOrderType.SELL).where('owner', '==', seller).get()
  ...
  it('Should buy in parallel', async () => {
    ...
    const buyFulfillmentCount = sales.reduce((sum, sale) => sum + (sale.type === TokenTradeOrderType.BUY ? sale.fulfilled : 0), 0)
    expect(buyFulfillmentCount).toBe(2 * tokenCount)
    const sellFulfillmentCount = sales.reduce((sum, sale) => sum + (sale.type === TokenTradeOrderType.SELL ? sale.fulfilled : 0), 0)
  ...
  it('Should buy and sell in parallel', async () => {
    ...
    await wait(async () => {
      const allSettled = (await admin.firestore()
        ...
        .where('owner', 'in', [seller, buyer])
        .get()
      ).docs.map(d => <TokenTradeOrder>d.data()).reduce((sum, act) => sum && act.status === TokenTradeOrderStatus.SETTLED, true)
      return allSettled
    })
    ...
    const allSettled = sales.reduce((sum, act) => sum && act.status === TokenTradeOrderStatus.SETTLED, true)
  ...
  it('Should cancel buy after half fulfilled', async () => {
    ...
    await wait(async () => {
      const snap = await admin.firestore().collection(COL.TOKEN_MARKET).where('owner', '==', buyer).where('type', '==', TokenTradeOrderType.BUY).get()
  ...
  it('Should cancel buy after half fulfilled, decimal values', async () => {
    ...
    const buyQuery = admin.firestore().collection(COL.TOKEN_MARKET)
      .where('type', '==', TokenTradeOrderType.BUY).where('owner', '==', buyer)
  ...
  it('Should settle after second run on more than batch limit', async () => {
    ...
    await wait(async () => {
      return (await admin.firestore().collection(COL.TOKEN_MARKET)
        .where('type', '==', TokenTradeOrderType.BUY).where('owner', '==', buyer).get())
  ...
  it.each([false, true])('Should not create royalty payments as percentage is zero', async (isMember: boolean) => {
    if (isMember) {
      ...
      await admin.firestore().doc(`${COL.SYSTEM}/${SYSTEM_CONFIG_DOC_ID}`).set({ tokenTradingFeePercentage: 0 })
    }
    mockWalletReturnValue(walletSpy, seller, { token: token.uid, price: MIN_IOTA_AMOUNT / 2, count: 2 * tokenCount, type: TokenTradeOrderType.SELL });
    await testEnv.wrap(tradeToken)({});
    ...
    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === (MIN_IOTA_AMOUNT / 2) * (2 * tokenCount))
    expect(billPaymentToSeller).toBeDefined()
  })
  ...
  it('Should create royalty payments only with dust', async () => {
    ...
    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === (MIN_IOTA_AMOUNT / 2) * (2 * tokenCount) + 800)
    expect(billPaymentToSeller).toBeDefined()
  ...
  it('Should fill buy and send dust to space one', async () => {
    ...
    const billPaymentToSpaceOne = billPayments.find(bp => bp.payload.amount === (MIN_IOTA_AMOUNT / 2) * (2 * tokenCount) * 0.01 * 0.1)
    expect(billPaymentToSpaceOne).toBeDefined()

    const billPaymentToSpaceTwo = billPayments.find(bp => bp.payload.amount === (MIN_IOTA_AMOUNT / 2) * (2 * tokenCount) * 0.01 * 0.9)
    expect(billPaymentToSpaceTwo).toBeDefined()

    const billPaymentToSeller = billPayments.find(bp => bp.payload.amount === (MIN_IOTA_AMOUNT / 2) * (2 * tokenCount) * 0.99 +
      (billPaymentToSpaceOne?.payload?.amount! < MIN_IOTA_AMOUNT ? billPaymentToSpaceOne?.payload?.amount! : 0) +
      (billPaymentToSpaceTwo?.payload?.amount! < MIN_IOTA_AMOUNT ? billPaymentToSpaceTwo?.payload?.amount! : 0))
    expect(billPaymentToSeller).toBeDefined()
  ...
  it('Should cancel after it needs higher fulfillment price', async () => {
    ...
    await wait(async () => {
      return (await buyQuery.get()).docs[0]?.data()?.status === TokenTradeOrderStatus.CANCELLED_UNFULFILLABLE
    })
